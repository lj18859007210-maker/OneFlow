...
const steps = ref([
  {
    label: "这个需求要解决什么问题？",
    placeholder:
      "请具体描述业务痛点，比如「客户投诉工单分派慢，客服需要手动从3个系统里查数据，每次耗时5分钟」",
    answer: "",
    nudge: "",
    state: "active",
  },
  ...
  },
  {
    label: "期望实现哪些核心功能？",
    placeholder:
      "列出1-3个核心功能点，具体到操作层面，比如「支持按投诉类型自动分派给对应部门，超时1小时自动升级至主管」",
    answer: "",
    nudge: "",
    state: "locked",
  },
  {
    label: "目前的替代方案是什么？",
    placeholder:
      "现在没有这个功能时，你们是怎么凑合解决的？比如「只能先把数据导出Excel手工算，每天浪费2小时」或者「目前没有替代方案，只能等」",
    answer: "",
    nudge: "",
    state: "locked",
  },
...
async function checkStep(i) {
  ...
  try {
    const qualityCheckPrompt = `你是一个耐心、专业的产品经理。用户正在填写需求表单的第${i + 1}个环节「${step.label}」，ta的回答如下：
