    ...
    <section class="ops-hero-card">
      <header class="ops-topbar">
        <div class="ops-title-wrap">
          ...
          <div class="ops-title-copy">
            <div class="ops-breadcrumb">需求列表 / 需求详情</div>
            <h1 class="ops-title">{{ requirement.title }}</h1>
          </div>
        </div>
        <div class="ops-actions">
          <button type="button" class="ops-btn ops-btn-primary" @click="goBack">
            返回
          </button>
        </div>
      </header>
      ...
      <div class="ops-summary-strip">
        <div
          class="ops-summary-item"
          :class="getPriorityClass(requirement.priority)"
        >
          <span>优先级</span
          ><strong>{{ requirement.priority || "未设置" }}</strong>
        </div>
        <div class="ops-summary-item score-card">
          <span>评分</span><strong>{{ formatScore(requirement.score) }}</strong>
        </div>
        ...
        <div class="ops-summary-item" :class="getStatusClass(effectiveStatus)">
          <span>状态</span><strong>{{ effectiveStatus || "-" }}</strong>
        </div>
      </div>
    </section>
    ...
    <main class="ops-workbench">
      <aside class="ops-left-rail">
        <section class="ops-panel">
          ...
          <div class="ops-flow-list">
            ...
            >
              <span class="ops-flow-dot">{{
                isStepCompleted(step, effectiveStatus) ? "✓" : index + 1
              }}</span>
              <div>
                <strong>{{ step }}</strong
                ><small>{{ getStatusHint(step) }}</small>
              </div>
            </div>
          </div>
        </section>
      ...
      <section class="ops-main-stack">
        <section class="ops-panel demand-panel">
          ...
          <div class="ops-description-grid">
            <article class="ops-description-block main">
              <h2>背景与目标</h2>
              <p>{{ requirement.description || "暂无描述" }}</p>
            </article>
            <article class="ops-description-block">
              <h2>需求内容</h2>
              <ol>
                <li>支持需求背景、描述、附件与状态集中查看。</li>
                <li>支持流程状态实时同步与操作记录沉淀。</li>
                <li>提供评论附件上传、预览、下载与归档能力。</li>
                <li>变更后实时生效，保留完整沟通链路。</li>
              </ol>
            </article>
            <article class="ops-description-block">
              <h2>验收标准</h2>
        ...
        <AttachmentCenter
          ...
          class="ops-panel ops-attachment-panel"
        />
            ...
            <table>
              ...
              <tbody>
                <tr v-if="comments.length === 0">
                  <td colspan="4" class="ops-empty-cell">还没有历史记录</td>
                </tr>
                <tr v-for="comment in comments" :key="comment.id">
                  ...
                  <td>
                    <template
                      v-for="(line, lineIndex) in getCommentLines(
                        comment.content,
                      )"
                      :key="comment.id + '-' + lineIndex"
                      ><span v-if="line.trim()">{{ line }}</span
                      ><br
                        v-if="
                          lineIndex <
                          getCommentLines(comment.content).length - 1
                        "
                    /></template>
                    <div
                    ...
                    >
                      <button
                        ...
                        "
                        type="button"
                        @click="openCommentAttachment(attachment)"
                      >
      ...
      <aside class="ops-right-rail">
        <section class="ops-panel action-panel">
          ...
          <div
            v-if="requirement?.approvalStatus === 'pending' && canApprove"
            class="ops-status-editor"
          >
          ...
          <div
            v-else-if="!isSubmitter && nextStatuses.length > 0"
            class="ops-status-editor"
          >
            <label>更新状态</label
            ><select v-model="newStatus" class="detail-select">
              <option value="">选择下一状态</option>
              <option v-for="step in nextStatuses" :key="step" :value="step">
                {{ step }}
              </option></select
            ><button
              type="button"
        ...
        <section class="ops-panel comment-panel">
          ...
          />
          <div
            v-if="pendingCommentAttachments.length > 0"
            class="detail-pending-grid"
          >
            <div
            ...
            >
              <img
                ...
                alt="待发送附件"
              />
              <div v-else class="detail-pending-file">
                {{ getAttachmentDisplayName(attachment) }}
              </div>
              <button
                type="button"
                class="detail-pending-remove"
          ...
          <div class="ops-comment-tools">
            <button
              type="button"
              class="ops-icon-btn small"
              title="添加附件"
            ...
            ><button
              type="button"
              class="detail-primary-btn send-btn"
              :disabled="
                (!chatMessage.trim() &&
    ...
    >
      <div class="detail-preview-controls">
        ...
        ><button
          ...
          @click="resetZoom"
        >
          {{ Math.round(imageZoomLevel * 100) }}%</button
        ><button
          type="button"
          class="detail-preview-close"
          title="关闭"
...
const currentUser = inject(
  "currentUser",
  ref({ name: "未登录", role: "user", permissions: [] }),
);
...
const canPreviewAttachments = computed(() =>
  hasPermission(currentUser.value, "attachment:preview"),
);
const canDownloadAttachments = computed(() =>
  hasPermission(currentUser.value, "attachment:download"),
);
const canPromoteAttachments = computed(() =>
  hasPermission(currentUser.value, "attachment:promote"),
);
const canUploadCommentAttachments = computed(() =>
  hasPermission(currentUser.value, "attachment:upload"),
);
...
const displayStatusOrder = computed(() => {
  ...
  return ordered.includes(effectiveStatus.value)
    ? ordered
    : [...ordered, effectiveStatus.value];
});
...
function isStepCompleted(step, currentStatus) {
  return (
    displayStatusOrder.value.indexOf(step) <
    displayStatusOrder.value.indexOf(currentStatus)
  );
}
...
function getAttachmentDisplayName(attachment) {
  return normalizeFileName(
    attachment?.originalName || attachment?.name || "附件",
  );
}
...
function handleImageZoom(event) {
  ...
  imageZoomLevel.value = Math.max(
    0.2,
    Math.min(3, imageZoomLevel.value + delta),
  );
}
...
async function loadWorkflow() {
  ...
  workflowTransitions.value = (transitionRes.data.data || []).filter(
    (item) => item.enabled,
  );
}
...
async function sendMessage() {
  if (
    ...
    pendingCommentAttachments.value.length === 0
  ) {
    showToast("请输入留言内容");
    return;
  }
  ...
  try {
    await commentApi.create({
      ...
      attachmentIds: pendingCommentAttachments.value
        .map((item) => item.id)
        .filter(Boolean),
    });
    chatMessage.value = "";
...
async function promoteCommentAttachment(attachment) {
  const category = window.prompt(
    "请输入归档分类：requirement / design / test-report / acceptance",
    "requirement",
  );
  if (!category) return;
  try {
...
.detail-ops-page {
  ...
  background:
    linear-gradient(
      180deg,
      rgba(231, 241, 252, 0.72),
      rgba(247, 250, 254, 0.96)
    ),
    repeating-linear-gradient(
      0deg,
      transparent 0 23px,
      rgba(18, 104, 216, 0.035) 23px 24px
    );
  font-size: 13px;
  line-height: 1.45;
}
...
.ops-panel {
  ...
  box-shadow: 0 8px 22px rgba(21, 74, 128, 0.08);
}
.ops-topbar {
  display: flex;
  align-items: center;
...
.ops-summary-strip {
  display: grid;
  grid-template-columns: 0.8fr repeat(4, 1fr) 0.9fr;
  gap: 6px;
  margin-top: 8px;
  padding: 8px;
  background: linear-gradient(180deg, #145c9e, #0e477e);
}
.ops-summary-item {
  min-width: 0;
  padding: 8px 10px;
...
.ops-panel-head {
  ...
  min-height: 34px;
  padding: 8px 10px;
  border-bottom: 1px solid var(--ops-soft);
  background: linear-gradient(180deg, #fbfdff, #f3f7fc);
}
.ops-panel-head h2,
.ops-description-block h2 {
  margin: 0;
  color: #143251;
  font-size: 13px;
  font-weight: 800;
...
.ops-flow-item:not(:last-child)::after {
  content: "";
  position: absolute;
  left: 11px;
  top: 28px;
...
.ops-flow-item.completed .ops-flow-dot {
  background: var(--ops-green);
}
.ops-flow-item.current .ops-flow-dot {
  background: var(--ops-blue);
  box-shadow: 0 0 0 3px rgba(18, 104, 216, 0.16);
}
...
.ops-kv-list.tight div {
  grid-template-columns: 72px minmax(0, 1fr);
}
.ops-kv-list div:last-child {
  border-bottom: 0;
}
.ops-kv-list dt {
  color: var(--ops-muted);
...
.ops-description-block {
  min-height: 120px;
  padding: 10px;
  border: 1px solid var(--ops-soft);
  border-radius: 5px;
  background: #fff;
}
.ops-description-block.main {
  grid-row: span 2;
}
.ops-description-block p {
  margin: 8px 0 0;
  color: #334f6c;
  white-space: pre-wrap;
}
.ops-description-block ol,
.ops-description-block ul {
  margin: 8px 0 0 16px;
  color: #334f6c;
}
.ops-description-block li + li {
  margin-top: 4px;
}
.ops-attachment-panel {
  min-height: 260px;
}
.ops-attachment-panel :deep(*) {
  border-radius: 5px !important;
}
.ops-attachment-panel :deep(.attachment-center),
.ops-attachment-panel :deep(.attachment-panel),
...
.detail-composer-input:focus {
  ...
  box-shadow: 0 0 0 2px rgba(18, 104, 216, 0.12);
}
.detail-primary-btn {
  min-height: 30px;
  padding: 0 12px;
...
.comment-panel .detail-pending-grid,
.comment-panel .ops-comment-tools {
  margin: 10px 10px 0;
  width: calc(100% - 20px);
}
.ops-comment-tools {
