  ...
  <div v-if="requirement" class="detail-page detail-ops-page">
    <section class="ops-hero-card">
      ...
      <div class="ops-summary-strip">
        ...
        <div class="ops-summary-item score-card"><span>评分</span><strong>{{ formatScore(requirement.score) }}</strong></div>
        <div class="ops-summary-item"><span>计划日期</span><strong>{{ requirement.actualDate ? formatDate(requirement.actualDate) : '未设置' }}</strong></div>
        <div class="ops-summary-item"><span>开发前用时</span><strong>{{ formatDurationAsDaysHours(requirement.lifecycleTiming?.preDevelopmentHours) }}</strong></div>
        <div class="ops-summary-item"><span>开发后用时</span><strong>{{ formatDurationAsDaysHours(requirement.lifecycleTiming?.postDevelopmentHours) }}</strong></div>
...
.detail-ops-page {
  ...
  padding: 12px;
  background:
    radial-gradient(circle at 18% 0, rgba(72, 141, 215, .16), transparent 28%),
...
.ops-hero-card {
  ...
  border-radius: 10px;
  background:
    linear-gradient(135deg, rgba(13, 39, 67, .98), rgba(14, 87, 143, .95) 58%, rgba(22, 119, 206, .92));
  box-shadow: 0 18px 46px rgba(17, 56, 96, .2);
}
...
.ops-hero-card .ops-btn-primary {
  ...
  background: #fff;
  color: #10578d;
  box-shadow: 0 10px 24px rgba(6, 29, 55, .18);
}
...
.ops-hero-card .score-card {
  ...
  background: linear-gradient(180deg, rgba(255, 218, 137, .22), rgba(255, 255, 255, .12));
}
