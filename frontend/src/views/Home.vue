      ...
      <table v-else class="tech-table">
        ...
        <tbody>
          <tr v-for="req in requirements" :key="req.id">
            ...
            <td>{{ req.platform || "-" }}</td>
            <td>
              <span class="tech-tag" :class="getPriorityClass(req.priority)">{{
                req.priority || "-"
              }}</span>
            </td>
            <td>
              <span class="tech-tag" :class="getStatusClass(req.status)">{{
                req.status
              }}</span>
            </td>
...
const canCreateRequirement = computed(() =>
  hasPermission(currentUser.value, "requirement:create"),
);
...
function updateSummaryFromResponse(payload) {
  ...
  scoreStats.value = payload.scoreStats || {
    "0-60": 0,
    "61-80": 0,
    "81-100": 0,
  };
...
async function loadDevelopers() {
  ...
  developerOptions.value = [
    ...new Set(developers.map((item) => item.name).filter(Boolean)),
  ];
}
...
async function loadRequirements(page = currentPage.value) {
  try {
    ...
    dashboardMetrics.value =
      dashboardResponse.data?.data || createEmptyDashboard();
  } catch (error) {
